...
export default function BlockSource({ cell, display, highlighted }) {
  const { cell_type: type, source = '', metadata } = cell;
  const theme = useTheme();
  const selectedStyle = theme === 'dark' ? dark : docco;

  const [isVisible, setIsVisible] = useState(
    display === -1 && metadata?.jupyter?.source_hidden ? false : true
  );

  if (!isVisible) return <div className="min-h-[20px] w-full" />;
  ...
  return (
    <div className="w-full my-4">
      <div
        className={`w-2 mr-2 rounded-sm cursor-pointer ${highlighted ? 'bg-blue-500 hover:bg-blue-700' : 'bg-transparent hover:bg-blue-300'}`}
        onClick={() => setIsVisible(!isVisible)}
      />
      {type === 'code' ? (
        <SyntaxHighlighter language="python" style={selectedStyle}>
          {sourceText}
        </SyntaxHighlighter>
      ) : (
        <div className="text-xl font-semibold text-gray-900 dark:text-white my-4">
